...
class LwsWebSocket final : public WebSocketTransport {
 ...
 private:
  ...
  static int Callback(struct lws *wsi, enum lws_callback_reasons reason,
    ...
    (void)user;
    auto *self = static_cast<LwsWebSocket *>(
        lws_context_user(lws_get_context(wsi)));
    return self == nullptr ? 0 : self->HandleCallback(wsi, reason, input,
                                                      length);
  }
    ...
    switch (reason) {
      case LWS_CALLBACK_CLIENT_APPEND_HANDSHAKE_HEADER: {
        ...
        if (AddHeader(wsi, cursor, end, "authorization:", auth) < 0 ||
            AddHeader(wsi, cursor, end, "protocol-version:",
                      std::to_string(version_)) < 0 ||
      ...
      case LWS_CALLBACK_CLIENT_WRITEABLE: {
        ...
        const int written = lws_write(
            wsi, frame.data() + LWS_PRE, message.data.size(),
      ...
      case LWS_CALLBACK_WS_PEER_INITIATED_CLOSE: {
        const auto *bytes = static_cast<const uint8_t *>(input);
        const unsigned int code = length >= 2
                                      ? (static_cast<unsigned int>(bytes[0])
                                             << 8) |
                                            bytes[1]
                                      : 0;
        const int reason_length = length > 2
                                      ? static_cast<int>(length - 2)
                                      : 0;
        std::fprintf(stderr,
                     "xiaozhi websocket: peer close code=%u reason=%.*s\n",
                     code, reason_length,
                     reason_length == 0
                         ? ""
                         : reinterpret_cast<const char *>(bytes + 2));
        break;
      }
